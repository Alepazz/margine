...
- **Il capitale sta fuori dai conti del mese; il resto del progetto no.** Dentro il tricount di una casa comprata convivono tre cose: gli esborsi di capitale — rogito, caparra, notaio — che con la spunta «fuori dai conti del mese» non entrano in margine, medie, confronti e saldo di ogni giorno; la rata del mutuo, che entra fra le **spese fisse** come l'affitto che sostituisce; e le spese correnti, che entrano come qualunque altra. Il progetto ha una pagina sua, con una barra che dice a che punto sono i rimborsi del capitale. [ADR-0079](docs/adr/0079-il-capitale-e-una-proprieta-della-spesa.md) · [ADR-0080](docs/adr/0080-la-barra-dei-rimborsi-misura-sul-lordo.md)
...
Dall'app si aggiunge una spesa col **+** al centro della barra — che nella pagina Prezzi registra invece una rilevazione, perché aggiunge la cosa della pagina in cui sei ([ADR-0044](docs/adr/0044-la-barra-serve-i-due-scopi.md)) — si corregge o si elimina dal foglio di dettaglio, si crea un viaggio, si registra un rimborso — dalla pagina Saldo in tutti e due i versi, o con un tocco dal Riepilogo se sei tu a dover pagare ([ADR-0062](docs/adr/0062-il-saldo-dice-il-verso-a-parole.md)), e si segna una spesa per il 730. Ogni modifica compare subito e viene committata nel repo in sottofondo, riscrivendo il file cifrato.
...
Dentro ci sta solo ciò che ha un contenuto. Una **correzione** fa riga solo se ha mosso la cifra — l'importo, la divisione, chi ha pagato: ritoccare un titolo, una categoria o una nota non manda una notifica, e nemmeno mettere la spunta 730 o welfare. E la **lista della spesa** collassa in una riga sola, «Ci sono cose nuove nella lista della spesa», per quante cose siano e da quanti commit: ogni cosa aggiunta è un commit suo, quindi la spesa di una settimana faceva venti righe e un pallino a `9+`. Prendere una cosa alla cassa non fa riga per niente. → [ADR-0094](docs/adr/0094-una-correzione-senza-cifra-non-e-una-novita.md), [ADR-0095](docs/adr/0095-la-lista-della-spesa-fa-una-riga-sola.md), [ADR-0091](docs/adr/0091-le-spunte-non-sono-novita.md)
