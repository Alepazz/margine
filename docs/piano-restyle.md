...
| Scheda | Anteprima |
|---|---|
| Saldo | il netto (es. −49,11 €) e in che verso |
...
3. **Due difetti che solo il browser poteva mostrare**: l'unità di misura che si appiccicava al prodotto successivo (serviva rimettere `unit` a `kg`, non solo `unitTouched` a falso — il piano l'aveva previsto a metà), e le schede dell'hub che sfondavano di 27px perché il minimo implicito di `1fr` è `auto` invece di zero.
